...
# NOTE: a truncated version of SHA-256 with a different set of initial values
...
proc update*[T](ctx: var Sha224Context, msg: openarray[T]) =
...
proc digest*(ctx: Sha224Context): array[28, uint8] =
  ## convert state array[8, uint32] to array[28, uint8]
...
proc hexDigest*(ctx: Sha224Context): string =
  ## convert state array[8, uint32] to hex string of length 56
  ## does not alter hash state
  var tempCtx: Sha224Context
  new tempCtx
  copyShaCtx(tempCtx, ctx)
...
proc newSha224Ctx*(msg: openarray[uint8] = @[]): Sha224Context =
...
proc newSha224Ctx*(msg: string): Sha224Context =
  return newSha224Ctx(msg.toOpenArrayByte(0, msg.len.pred))
